...
export default async function ContractorMessagesPage() {
  ...
  const threads = await prisma.serviceRequest
    .findMany({
      where: {
        service: { contractorId: session?.user?.id ?? "" },
        status: { in: [...CHAT_OK] },
      },
    ...
    <div className="space-y-6 p-4 lg:p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Chat with customers on accepted jobs
        </p>
      </div>
      ...
      {threads.length === 0 ? (
        <div className="rounded-2xl border border-gray-100 bg-white p-14 text-center shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-400">
            Accept a customer request to unlock messaging.
          </p>
          <Link
            href="/construction/requests"
        ...
        <div className="space-y-2">
          {threads.map((t) => {
            const name = t.client.name.split(" — ")[0];
            const last = t.messages[0];
            return (
              <Link
                key={t.id}
                href={`/construction/messages/${t.id}`}
                className="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition hover:border-orange-200 hover:shadow-md"
              >
